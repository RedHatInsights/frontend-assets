...
function generateTSXFileContent(metadata) {
  ...
  // Extract background metadata from SVG desc tag
  const darkBackgroundMatch = svgContent.match(/<desc>background:dark<\/desc>/);
  const backgroundMetadata = darkBackgroundMatch ? 'dark' : null;
  ...
  try {
    ...
    const backgroundMetadataExport = backgroundMetadata 
...
/**
 * @type {Record<string, 'dark' | 'light' | null>}
 */
const backgroundMetadataMap = {};
...
/**
 * Write background metadata to a JavaScript file
 */
async function writeBackgroundMetadataFile() {
  const metadataFilePath = path.join(projectBase, 'src', 'backgroundMetadata.js');
  // Format with single quotes for ESLint compatibility
  // Add trailing comma before closing brace for ESLint
  const formattedMetadata = JSON.stringify(backgroundMetadataMap, null, 2)
    .replace(/"/g, "'")
    .replace(/\n\}$/, ',\n}');
...
export const backgroundMetadata = ${formattedMetadata};
`;
  
  await promisiFiedWriteFile(metadataFilePath, metadataContent);
  console.log('✅ Generated background metadata file: src/backgroundMetadata.js');
}
...
async function processReactIcons(svgFiles) {
  ...
  for (const file of svgFiles) {
    ...
    // Extract background metadata from the SVG file
    const svgContent = fs.readFileSync(file, 'utf8');
    const darkBackgroundMatch = svgContent.match(/<desc>background:dark<\/desc>/);
    const backgroundMetadata = darkBackgroundMatch ? 'dark' : null;
    
    // Store in the map using the component path (without extension)
    // Use ./ prefix to match how paths are used in the demo
    const componentPath = './' + metadata.sourceFilePath.replace(/^\//, '').replace(/\.tsx$/, '');
    backgroundMetadataMap[componentPath] = backgroundMetadata;
  ...
  await writeBackgroundMetadataFile();
}
